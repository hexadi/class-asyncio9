...
async def publish_message(w, client, app, action, name, value):
    ...
    await client.publish(f"v1cd/{app}/{action}/{student_id}/model-01/{w.SERIAL}"
...
async def CoroWashingMachine(w, client):
    ...
    while True:
        ...
        if w.MACHINE_STATUS == 'ON':
            ...
            await publish_message(w, client, "hw", "get", "status", "STOPPED")

            await publish_message(w, client, "hw", "get", "status", "POWER OFF")
            w.MACHINE_STATUS = 'OFF'
